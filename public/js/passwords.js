...
(function (global) {
  ...
  function activeSymbols() { return state.symbolSet || CHARS.symbol; }
  ...
  function buildCharset() {
    ...
    var arr = chars.split('');
    if (optNoAmbig.checked) arr = arr.filter(function (c) { return AMBIG.indexOf(c) < 0; });
    if (optNoDb.checked) arr = arr.filter(function (c) { return DBUF.indexOf(c) < 0; });
  ...
  function enforceClasses(pwd, charset) {
    ...
    function present(src) {
      return src.split('').filter(function (c) { return charset.indexOf(c) >= 0; });
    }
    ...
    classes.forEach(function (cls) {
      ...
      var has = arr.some(function (c) { return cls.indexOf(c) >= 0; });
  ...
  function init() {
    ...
    optUpper = U.$('pw-upper'); optLower = U.$('pw-lower');
    optDigit = U.$('pw-digit'); optSymbol = U.$('pw-symbol');
    optNoAmbig = U.$('pw-no-ambig'); optNoDb = U.$('pw-no-db');
    ...
    [optUpper, optLower, optDigit, optSymbol, optNoAmbig, optNoDb].forEach(function (c) {
      c.addEventListener('change', function () { clearPreset(); generate(); });
    });

    batchBtns.forEach(function (b) {
      b.addEventListener('click', function () {
        state.batch = parseInt(b.dataset.count, 10);
        batchBtns.forEach(function (x) { x.classList.toggle('active', x === b); });
        generate();
      });
    });

    U.$('pw-regen').addEventListener('click', generate);
    U.bindCopy(U.$('pw-copy'), function () {
      return U.$$('.pwd-row', out).map(function (r) { return r.textContent; }).join('\n');
    }, U.$('pw-copy-label'));

    U.$('pw-reveal').addEventListener('click', function () {
      state.revealed = !state.revealed;
      U.$$('.pwd-row', out).forEach(function (r) { r.classList.toggle('hidden', !state.revealed); });
  ...
  U.ready(init);
  global.PwdPasswords = { PRESETS: PRESETS };
